...
|------|-----------|---------------|
| CloudWatch Agent | DaemonSet（各ノードに1つ） | **Deployment（共有サービスとして稼働）** |
| OTel エンドポイント | `cloudwatch-agent.amazon-cloudwatch:4316` | `cloudwatch-agent.amazon-cloudwatch:4316`（同じ） |
...
| RUM | ✅ make コマンドで有効化 | ✅ **手動で Fargate 用 k8s を更新して有効化** |
...
| 実ユーザー監視 | CloudWatch RUM | ✅（手動設定が必要） |
...
**ログロググループ:**
- EC2 環境: `/obs-poc/eks-ec2-appsignals/application`
...
ただし、現時点の PoC では **Fargate 専用の enable コマンドが未実装**のため、以下の手順で手動有効化する。
...
# Fargate の netwatch-ui Deployment に RUM 環境変数を設定
kubectl set env deployment/netwatch-ui -n eks-fargate-appsignals \
  CW_RUM_APP_MONITOR_ID=<UUID> \
  CW_RUM_IDENTITY_POOL_ID=ap-northeast-1:<UUID> \
  CW_RUM_REGION=ap-northeast-1

# ロールアウト再起動
kubectl rollout restart deployment/netwatch-ui -n eks-fargate-appsignals
kubectl rollout status deployment/netwatch-ui -n eks-fargate-appsignals --timeout=120s
```
