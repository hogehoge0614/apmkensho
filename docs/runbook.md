...
- EKS クラスター: `obs-poc`、ネームスペース: `demo-ec2` でフィルタリングする
...
フィルター: クラスター `obs-poc`、ネームスペース `demo-ec2`
...
```bash
# kubectl exec 経由で確認
kubectl exec -n demo-ec2 -it deployment/device-api -- \
  python3 -c "
...
または Web UI の Chaos 画面（`http://<EC2_BASE>/chaos`）でも確認可能。
...
ブラウザで `http://<EC2_BASE>/chaos` を開き、「Reset All Chaos」ボタンをクリックする。
...
**Pod の再起動（device-api）:**
```bash
kubectl rollout restart deployment/device-api -n demo-ec2
# ロールアウト完了を待機
kubectl rollout status deployment/device-api -n demo-ec2
```

**Pod の再起動（netwatch-ui）:**
```bash
kubectl rollout restart deployment/netwatch-ui -n demo-ec2
kubectl rollout status deployment/netwatch-ui -n demo-ec2
```

**Pod の再起動（metrics-collector）:**
```bash
kubectl rollout restart deployment/metrics-collector -n demo-ec2
kubectl rollout status deployment/metrics-collector -n demo-ec2
```

**Pod の再起動（alert-api）:**
```bash
kubectl rollout restart deployment/alert-api -n demo-ec2
kubectl rollout status deployment/alert-api -n demo-ec2
```
...
- [ ] device-api Pod が正常に Running しているか（`kubectl get pods -n demo-ec2`）
...
3. alert-api の `/alerts` エンドポイントに直接アクセスし、アラート件数を確認する:
   ```bash
   kubectl exec -n demo-ec2 -it deployment/alert-api -- \
     python3 -c "
   import urllib.request, json
...
4. kubectl で対象サービスの Pod が Running かどうかを確認する:
   ```bash
   kubectl get pods -n demo-ec2
   ```
5. LoadBalancer の External IP が変わっていないか確認する:
   ```bash
   kubectl get svc -n demo-ec2 netwatch-ui
   ```
...
kubectl describe pod -n demo-ec2 <pod-name>

# ログ確認（直近 50 行）
kubectl logs -n demo-ec2 -l app=device-api --tail=50

# ログ確認（netwatch-ui）
kubectl logs -n demo-ec2 -l app=netwatch-ui --tail=50

# ログ確認（metrics-collector）
kubectl logs -n demo-ec2 -l app=metrics-collector --tail=50

# ログ確認（alert-api）
kubectl logs -n demo-ec2 -l app=alert-api --tail=50

# ログをリアルタイムで追う
kubectl logs -n demo-ec2 -l app=device-api -f
...
# デプロイメント一覧確認
kubectl get deployments -n demo-ec2
```
